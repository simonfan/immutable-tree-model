...
const getChildren = _pipe(childIds, getNodes)
...
const getDescendantIds = (state, nodeId) => {
	return getChildIds(state, nodeId).reduce((acc, childId) => {
		return getNode(state, childId).type === 'branch' ?
			[...acc, childId, ...getDescendantIds(state, childId)] :
			[...acc, childId]
	}, [])
}
const getDescendants = _pipe(getChildIds, getNodes)
